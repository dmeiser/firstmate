...
FM_COMPOSER_KIMI_FOOTER_RE=   # optional fleet-wide regex override for the composer-furniture rows the empty-composer staleness walk steps over below the box; the default recognizes only kimi 0.40.1's rendered permission-badge and context-meter footer shapes, never arbitrary text below a box (bin/fm-composer-lib.sh)
